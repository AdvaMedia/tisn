...
module Taggable
  ...
  module ClassMethods
    ...
    def tag_descriptions(hash = nil)
      Taggable.tag_descriptions[self.name] ||= (hash ||{})
    end

    def desc(text)
      Taggable.last_description = RedCloth.new(Util.strip_leading_whitespace(text)).to_html
    end

    def tag(name, &block)
      self.tag_descriptions[name] = Taggable.last_description if Taggable.last_description
      Taggable.last_description = nil
